...
def get_neigborhood(geocode) -> str:
    """ Obtain the neigborhood from the Portland Maps API """
    ...
    try:
        ...
        neigborhood = data.get("features")[0].get("attributes").get("NAME").title()

    except Exception as e:
        logger.error_log( f'Error Getting the neigborhood: {e}' )
        exit()
